...
func generate_walls():
	var wall_height_offset = 100
	# hotfix to avoid that at the edge-vertices # a lizard can fall off
	# var offset_x = 0

	var min_x : float = aabb_node.position.x 
	var min_y : float = aabb_node.position.y - 3
	var min_z : float = aabb_node.position.z

	var max_x : float = aabb_node.end.x
	var max_y : float = aabb_node.end.y + wall_height_offset
	var max_z : float = aabb_node.end.z 
	
	 # First Wall
	var p1 : Vector3 = Vector3(max_x, min_y, min_z)
	var p2 : Vector3 = Vector3(max_x, min_y, max_z)
	var p3 : Vector3 = Vector3(max_x, max_y, max_z)
	var p4 : Vector3 = Vector3(max_x, max_y, min_z)
	...
	shape_3.points = PackedVector3Array([p1,p2,p3,p4])
	...
	p3 = Vector3(min_x, max_y, min_z)
	p4 = Vector3(max_x, max_y, min_z)


	var shape_4 = ConvexPolygonShape3D.new()
	shape_4.points = PackedVector3Array([p1,p2,p3,p4])
